...
class DockIt extends Commands	{
	...
	_SetChildNoFull(){
		try{
			if (This.Child.Positions.Start.Style & 0x01000000) {  ; WS_MAXIMIZE = 0x01000000
				...
				;Windows全屏样式有特殊点  不能采用0, 0, 1920, 1080  应采用-8, -8, 1920+16, 1080+16
	...
	_CloseWindow(){
		...
		try{
			...
			; 获取窗口的原始宽度和高度
			originalWidth := This.Child.Positions.Start.W
			originalHeight := This.Child.Positions.Start.H
			...
			if (This.Child.Positions.Start.Style & 0x01000000) {  ; WS_MAXIMIZE = 0x01000000
			; 判断窗口是否为全屏
			;if (ABS(originalWidth - A_ScreenWidth)<100 and ABS(originalHeight == A_ScreenHeight)<100) {
				; 计算窗口的新大小
